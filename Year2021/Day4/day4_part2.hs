...
runBingoLottery step lotteryNumbers cardList = do
    ...
    let _notWinnerMatcher (cardSum, card) = not $ any ((== -5) . sum) card || any ((== -5) . sum) (transpose card)
    -- Remove the winners from the list of bingo cards
    let nonWinners = filter _notWinnerMatcher updatedBingoCards
    -- If there is no bingo card remaining return the score for the final card
    if null nonWinners then
        trace ("final step: " ++ show step) drawnNumber * fst (head updatedBingoCards)
    else -- Else keep running the bingo lottery recursively with the remaining bingo cards
        trace ("step: " ++ show step) runBingoLottery (step + 1) (tail lotteryNumbers) nonWinners
