...
main = do
    ...
    -- Remove diagonal lines by checking if initial x and y values are the same as the final x and y values
    let straightLines = filter (\((x1, y1),(x2, y2)) -> (x1 == x2) || (y1 == y2)) ventTupleList
    -- Create a grid containing all the lines and the amount of overlap for each coordinate
    let grid = fillGrid straightLines
